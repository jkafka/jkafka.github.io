...
This code shows how to apply our SML tool (hereafter referred to as "the IPV Classifier") to suicide data in the National Violent Death Reporting System (NVDRS). The IPV Classifier is designed to detect whether suicide cases in NVDRS were precipitated by intimate partner violence (IPV) or not (yes/no), using textual information from NVDRS death narratives. 
...
nvdrs_clean$narr <- gsub('[ÃåââãåÅâÂ]', '', nvdrs_clean$narr)
nvdrs_clean$narr <- gsub("[\n]",       " ", nvdrs_clean$narr)
...
nvdrs_clean <- 	nvdrs_clean %>% 
	select(id, state, incidentyear, incidentid, personid, 
				 narrativecme, narrativele, starts_with("vdrs_"), starts_with("yr_"))
...
If there are any NVDRS variables with missing values, you can review them on a case-by-case to confirm that missing should be set to zero vs. dropped from the analytical sample. 
...
There you have it, we now have one prediction per case. Thanks for walking through this demo with me. Best of luck with your own research, data science work, or applied public health practice!
